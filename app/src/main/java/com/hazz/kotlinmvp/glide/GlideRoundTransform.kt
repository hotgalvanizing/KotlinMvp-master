 ...
 * desc:
 * 1.永远不要把transform()传给你的原始resource或原始Bitmap给recycle()了，
 *      更不要放回BitmapPool，因为这些都自动完成了。
 *      值得注意的是，任何从BitmapPool取出的用于自定义图片变换的辅助Bitmap，
 *      如果不经过transform()方法返回，就必须主动放回BitmapPool或者调用recycle()回收。
 * 2.如果你从BitmapPool拿出多个Bitmap或不使用你从BitmapPool拿出的一个Bitmap，
 *      一定要返回extras给BitmapPool。
 * 3.如果你的图片处理没有替换原始resource
 *      (例如由于一张图片已经匹配了你想要的尺寸，你需要提前返回),
